...
preload-scraper.js  poll() [configurable interval, default 2 min; guarded against
  ...
  → ipcRenderer.send('usage:update', { bars, grants, credits, unknownKeys, planName, raw, fetchedAt })
  → ipcMain (main.js)
...
- `five_hour.resets_at` is **not** proof that a session window belongs to this machine: the API keeps reporting the window of the last account-wide activity, so a Mac that just booted can receive a live `resets_at` alongside `utilization: 0` (window opened earlier from a phone, another browser, or before the machine was switched on). The widget therefore only shows the big countdown while `five_hour.utilization > 0` (`sessionCountdownMs()` in `renderer.js`); at 0% it reads `--:--` and the exact reset time stays in the SESSION row tooltip.
- **Scoped dollar grants**: a bucket with a numeric `limit_dollars` (e.g. `iguana_necktie` = the $100 Claude Code cloud-sessions gift, live since 2026-09-25: `{limit_dollars:100, used_dollars, remaining_dollars, resets_at:<window end>}`) is not a % limit and is not part of the `/prepaid/credits` tranches. `normalize()` routes it to `data.grants` (labels in `GRANT_LABEL_MAP`) and `renderGrants()` shows it as a money row under CREDITS — never sum it into the CREDITS balance.
- The API also exposes several always-`null` codename fields (`tangelo`, `omelette_promotional`, `copper_kite`, etc.) reserved for buckets not enabled on the polling account. Don't invent labels for these; `collectUnknownKeys()` will surface one the moment it goes non-null with an unrecognized shape.
